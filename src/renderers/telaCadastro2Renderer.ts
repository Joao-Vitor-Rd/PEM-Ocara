...
// --- DOMContentLoaded: PREENCHIMENTO E LÓGICA DE VISUALIZAÇÃO ---
window.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    if (dadosCasoJSON) {
      ...
      setCheckboxValues(
        'input[id^="q02-agressao"]',
        dadosCaso._agressoesGraves || []
      );
      setCheckboxValues(
        'input[id^="q03-agressao"]',
        dadosCaso._outrasAgressoes || []
      );
      setRadioValue("q04-estupro", dadosCaso._estupro);
      setCheckboxValues(
        'input[id^="q05-comportamento"]',
        dadosCaso._comportamentos || []
      );
      ...
      setCheckboxValues(
        'input[id^="q08-uso-abusivo"]',
        dadosCaso._usoDrogas || []
      );
      ...
      setRadioValue(
        "q12-desempregado-dificuldades",
        dadosCaso._desempregadoDificuldades
      );
      setRadioValue("q13-acesso-armas", dadosCaso._acessoArmas);
      setCheckboxValues(
        'input[id^="q14-ameacou-agrediu"]',
        dadosCaso._ameacouAgrediu || []
      );

      // Bloco III - Sobre Você
      setRadioValue("q15-separacao", dadosCaso.separacaoRecente);
      setRadioValue(
        "q17-novo-relacionamento",
        dadosCaso.novoRelacionamentoAumentouAgressao
          ? "Sim"
          : dadosCaso.novoRelacionamentoAumentouAgressao === false
          ? "Não"
          : undefined
      );
      ...
      // Q16 - Filhos
      setCheckboxById(
        'input[id^="q16-tem-filhos"]',
        dadosCaso._temFilhosIds || []
      );
      if (dadosCaso._q16QuantosAgressor)
        setInputValue("q16-quantos-agressor", dadosCaso._q16QuantosAgressor);
      if (dadosCaso._q16QuantosOutro)
        setInputValue("q16-quantos-outro", dadosCaso._q16QuantosOutro);
      setCheckboxValues(
        'input[id^="q16-1-faixa-etaria"]',
        dadosCaso._q16FaixaEtariaIds || []
      );
      setRadioValue("q16-2-deficiencia", dadosCaso._q16Deficiencia);
      if (dadosCaso._q16QuantosDeficiencia)
        setInputValue(
          "q16-2-quantos-deficiencia",
          dadosCaso._q16QuantosDeficiencia
        );
      setCheckboxValues(
        'input[id^="q16-3-conflito"]',
        dadosCaso._q16ConflitosIds || []
      );
      setRadioValue("q16-4-presenciaram-violencia", dadosCaso._q16Presenciaram);
      setRadioValue(
        "q16-5-violencia-gravidez",
        dadosCaso._q16ViolenciaGravidez
      );

      setRadioValue("q18-deficiencia", dadosCaso.possuiDeficienciaDoenca);
      if (
        dadosCaso.possuiDeficienciaDoenca &&
        dadosCaso.possuiDeficienciaDoenca !== "Sim" &&
        dadosCaso.possuiDeficienciaDoenca !== "Não"
      ) {
        setInputValue(
          "q18-qual-deficiencia",
          dadosCaso.possuiDeficienciaDoenca
        );
      }

      if (dadosCaso.corRaca) setRadioValue("cor", dadosCaso.corRaca);
      if (dadosCaso._moraEmAreaRisco)
        setRadioValue("q20-local-risco", dadosCaso._moraEmAreaRisco);
      if (dadosCaso._dependenteFinanceira)
        setRadioValue(
          "q21-dependente-financeira",
          dadosCaso._dependenteFinanceira
        );
      if (dadosCaso._abrigamentoTemporario)
        setRadioValue("q22-abrigamento", dadosCaso._abrigamentoTemporario);
    }
  } catch (error) {
    console.error("Erro ao prefill telaCadastro2:", error);
  }

  // --- LÓGICA DO MODO DE VISUALIZAÇÃO (CORRIGIDA PARA TELA INTERMEDIÁRIA) ---
  const isModoVisualizacao =
    sessionStorage.getItem("modoVisualizacao") === "true";

  if (isModoVisualizacao) {
    console.log("[telaCadastro2] Modo de Visualização Ativado");

    // 1. Bloqueia todos os campos
    ...
    // 2. Esconde o botão "Próximo" (de salvamento)
    if (btnProximo) {
      btnProximo.style.display = "none";
    }

    // 3. Altera o botão Voltar para ser o "Continuar" na visualização
    if (btnVoltar) {
      btnVoltar.innerText = "Continuar Visualização >>";

      // --- CÓDIGO DE CLONAGEM (REQUERIDO) ---
      // Clonamos e substituímos para remover o listener global de salvar/voltar
      const novoVoltarBtn = btnVoltar.cloneNode(true) as HTMLButtonElement;
      if (btnVoltar.parentNode) {
        btnVoltar.parentNode.replaceChild(novoVoltarBtn, btnVoltar);
      }
      // --- FIM DO CÓDIGO DE CLONAGEM ---

      // NOVO COMPORTAMENTO: Ação de 'Próximo' na visualização: Ir para telaCadastro3
      novoVoltarBtn.addEventListener("click", async (event) => {
...
// Este listener só é executado se o modo de visualização estiver DESLIGADO.
btnVoltar.addEventListener("click", async () => {
  try {
    ...
    dadosCaso._comportamentos = getCheckedValues(
      'input[id^="q05-comportamento"]'
    );
    ...
    dadosCaso._desempregadoDificuldades = getRadioValue(
      "q12-desempregado-dificuldades"
    );
    dadosCaso._acessoArmas = getRadioValue("q13-acesso-armas");
    dadosCaso._ameacouAgrediu = getCheckedValues(
      'input[id^="q14-ameacou-agrediu"]'
    );
    ...
    dadosCaso.novoRelacionamentoAumentouAgressao = novoRelac
      ? novoRelac === "Sim"
      : undefined;
    ...
    dadosCaso._q16FaixaEtariaIds = getCheckedIds(
      'input[id^="q16-1-faixa-etaria"]'
    );
    dadosCaso._q16Deficiencia = getRadioValue("q16-2-deficiencia");
    dadosCaso._q16QuantosDeficiencia = getInputValue(
      "q16-2-quantos-deficiencia"
    );
...
btnProximo.addEventListener("click", async () => {
  try {
    ...
    const desempregadoDificuldades = getRadioValue(
      "q12-desempregado-dificuldades"
    );
    ...
    const q16FaixaEtariaIds = getCheckedValues(
      'input[id^="q16-1-faixa-etaria"]'
    );
    ...
    if (
      !q16Deficiencia &&
      temFilhosIds.length > 0 &&
      !temFilhosIds.includes("q16-tem-filhos-nao")
    ) {
    ...
    dadosCaso.temFilhosComAgressor = temFilhosIds.some((id) =>
      id.includes("agressor")
    );
    dadosCaso.qntFilhosComAgressor = dadosCaso.temFilhosComAgressor
      ? parseInt(q16QuantosAgressor) || 0
      : 0;
    dadosCaso.temFilhosOutroRelacionamento = temFilhosIds.some((id) =>
      id.includes("outro")
    );
    dadosCaso.qntFilhosOutroRelacionamento =
      dadosCaso.temFilhosOutroRelacionamento
        ? parseInt(q16QuantosOutro) || 0
        : 0;
    dadosCaso.faixaFilhos = q16FaixaEtariaIds || [];
    dadosCaso.filhosComDeficiencia = parseInt(q16QuantosDeficiencia) || 0;
    dadosCaso.conflitoAgressor =
      q16ConflitosIds.length > 0 ? q16ConflitosIds.join("; ") : "";
