...
const cadastroExistenteBtn = document.getElementById(
  "telaCadastroExistente"
) as HTMLButtonElement;
const assistidasBtn = document.getElementById(
  "listarAssistidas"
) as HTMLLIElement;
const redeApoioBtn = document.getElementById("telaRedeApoio") as HTMLLIElement;
const estatisticasBtn = document.getElementById(
  "telaEstatisticas"
) as HTMLLIElement;
const contaBtn = document.getElementById("telaConta") as HTMLLIElement;
const modalSelectOverlay = document.getElementById(
  "modalSelectOverlay"
) as HTMLElement;
const closeSelectModal = document.getElementById(
  "closeSelectModal"
) as HTMLElement;
const assistidaListContainer = document.querySelector(
  ".assistida-list-container"
) as HTMLElement;
const btnAdicionarCaso = document.getElementById(
  "btnAdicionarCaso"
) as HTMLButtonElement;
    ...
    if (resultado.success && resultado.assistidas) {
      ...
      resultado.assistidas.forEach((assistida: any, index: number) => {
        ...
        label.innerHTML = `
                    ...
                    <span class="item-text">${assistida.id}: ${
          assistida.nome
        }</span>
                `;
  ...
  btnAdicionarCaso.addEventListener("click", async () => {
    ...
    if (radioSelecionado) {
      ...
      if (resultado.success && resultado.assistida) {
        ...
        console.log(
          "[TelaInicial] Dados da assistida recebidos do banco:",
          resultado.assistida
        );
        ...
        console.log(
          "[TelaInicial] sessionStorage.dadosAssistida:",
          sessionStorage.getItem("dadosAssistida")
        );
...
if (cadastroAssistidaBtn) {
  cadastroAssistidaBtn.addEventListener("click", async (event) => {
    ...
    // **LIMPEZA TOTAL DE FLAGS:** Garante que o formulário abra vazio e editável.
