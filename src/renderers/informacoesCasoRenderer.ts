...
import {
  ...
  navigateToTelaEstatisticas,
} from "../utils/SidebarManager.js";

export {};

const telaInicialBtn = document.getElementById(
  "telaInicial"
) as HTMLLIElement | null;
const cadastroAssistidaBtn = document.getElementById(
  "telaCadastroAssistida"
) as HTMLButtonElement | null;
const listarAssistidasBtn = document.getElementById(
  "listarAssistidas"
) as HTMLLIElement | null;
const telaRedeApoioBtn = document.getElementById(
  "telaRedeApoio"
) as HTMLLIElement | null;
const telaEstatisticasBtn = document.getElementById(
  "telaEstatisticas"
) as HTMLLIElement | null;
const nomeAssistida = document.getElementById(
  "Assistida"
) as HTMLSpanElement | null;
const nomeAgressor = document.getElementById(
  "Agressor"
) as HTMLSpanElement | null;
const dataAberturaCaso = document.getElementById(
  "DataCadastro"
) as HTMLSpanElement | null;
const tipoViolencia = document.getElementById(
  "TipoViolencia"
) as HTMLSpanElement | null;
...
const Config = {
  ...
  ICONS: {
    ...
    mp4: "./icons/mov.png",
    m4a: "./icons/mov.png",
    zip: "./icons/fecho-eclair.png",
    rar: "./icons/fecho-eclair.png",
...
class FileManager {
  ...
  buscarPorId(id: any) {
    return [...this.state.prova, ...this.state.relatorio].find(
      (f: any) => f.id === id
    );
  }
...
class UIManager {
  ...
  mostrarConfirmacao(mensagem: any) {
    return new Promise((resolve) => {
      ...
      const popupMensagem = document.getElementById(
        Config.POPUP_CONFIRMAR.MENSAGEM
      );
      const btnCancelar = document.getElementById(
        Config.POPUP_CONFIRMAR.BTN_CANCELAR
      );
      const btnConfirmar = document.getElementById(
        Config.POPUP_CONFIRMAR.BTN_CONFIRMAR
      );
    ...
    arquivos.forEach((arquivo: any) => {
      ...
      if (arquivo.status === "upando") {
        ...
        const iconeVisibilidade =
          arquivo.visibilidade && arquivo.visibilidade === "publico"
            ? "visibility"
            : "visibility_lock";
        console.log(
  ...
  configurarEventDelegation(callbacks: any) {
    ...
    Object.values(this.containers).forEach((container: any) => {
      ...
      if ((container as any)._downloadHandler) {
        container.removeEventListener(
          "click",
          (container as any)._downloadHandler
        );
      }
  ...
  renderizarSelecaoModal(
    todosArquivos: any,
    selecionadosIds: any,
    onToggle: any
  ) {
    const listaModal = document.getElementById("listaAnexosModal");
    const labelNome = document.getElementById(
      "nome-anexo-modal"
    ) as HTMLElement | null;
    ...
    const itens = [
      { id: "formulario", nome: "Formulário", fixo: true },
      ...todosArquivos.filter((a: any) => a.status === "upado"),
    ];
    ...
    itens.forEach((item) => {
      ...
      div.innerHTML = `
                <div class="anexo-nome-modal" title="${
                  item.nome
                }">${Formatters.truncarNome(item.nome, 35)}</div>
    ...
    if (selecionadosIds.length === 0 && labelNome)
      labelNome.textContent = "Nenhum anexo selecionado";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const informacoesGerais = await window.api.getInformacoesGeraisDoCaso(
    Number(idCaso)
  );
  ...
  const acoesArquivo = {
    onDelete: async (id: any) => {
      console.log(
        "🗑️ Iniciando deleção do arquivo com ID:",
        id,
        "tipo:",
        typeof id
      );
      ...
      const confirmar = await uiManager.mostrarConfirmacao(
      ...
      if (confirmar) {
        try {
          ...
          console.log(
            "📋 Nome do arquivo recuperado do sessionStorage:",
            nomeArquivoSalvo
          );

          // PASSO 2: Deletar do banco (passando o nome salvo)
          console.log("Deletando do banco...");
          const resultadoDeletar = await window.api.excluirAnexo(
            id,
            nomeArquivoSalvo
          );
          ...
          if (resultadoDeletar?.success) {
            ...
            try {
              ...
              const usuarioLogado = usuarioLogadoJSON
                ? JSON.parse(usuarioLogadoJSON)
                : null;
              const emailFuncionario =
                usuarioLogado?.email || "sistema@sistema.com";
              ...
              console.log(
                "✅ Histórico registrado: Arquivo deletado por",
                nomeFuncionario
              );
            } catch (erroHistorico) {
              console.warn(
                "Aviso: Histórico não foi registrado:",
                erroHistorico
              );
            }
  ...
  const atualizarTela = () => {
    uiManager.renderizarLista(
      "prova",
      fileManager.obterTodos("prova"),
      acoesArquivo
    );
    uiManager.renderizarLista(
      "relatorio",
      fileManager.obterTodos("relatorio"),
      acoesArquivo
    );
  };
      ...
      const mensagem =
        arquivosInvalidos.length === 1
          ...
          : `${
            ...
            } arquivo(s) excedem o tamanho máximo permitido de 100MB:\n${arquivosInvalidos.join(
              ", "
            )}`;
  ...
  // Botão Acessar Formulário (MODO VISUALIZAÇÃO)
  const btnAcessarForm = document.getElementById("acessar-form");
  if (btnAcessarForm) {
    btnAcessarForm.addEventListener("click", async () => {
      console.log("Acessar formulário (Visualização) clicado");

      if (!idAssistida) {
        console.error("ID da assistida não disponível");
        uiManager.mostrarPopup(
          "Erro: Não foi possível identificar a assistida."
        );
        return;
      }

      try {
        // 1. Busca os dados completos da assistida para preencher o formulário
        // (Assumindo que sua API tem essa função, pois foi usada na tela anterior)
        const resultado = await window.api.buscarAssistidaPorId(idAssistida);

        if (resultado.success && resultado.assistida) {
          // 2. Salva os dados no storage para a telaCadastro1 ler
          sessionStorage.setItem(
            "dadosAssistida",
            JSON.stringify(resultado.assistida)
          );

          // 3. ATIVA O MODO VISUALIZAÇÃO (Importante!)
          sessionStorage.setItem("modoVisualizacao", "true");

          // 4. Abre a tela de cadastro (que vai se comportar como visualização)
          await window.api.openWindow("telaCadastroAssistida");
        } else {
          console.error("Erro ao buscar dados:", resultado);
          uiManager.mostrarPopup(
            "Não foi possível carregar os dados completos para visualização."
          );
        }
      } catch (error) {
        console.error("Erro no processo de visualização:", error);
        uiManager.mostrarPopup("Erro ao abrir o formulário.");
      }
    });
  }
  ...
  if (btnEditarForm) {
    btnEditarForm.addEventListener("click", () => {
      const modalEditarFormulario = document.getElementById(
  ...
  if (btnAlterarPrivacidade) {
    btnAlterarPrivacidade.addEventListener("click", () => {
      const modalPrivacidadeFormulario = document.getElementById(
        "modalPrivacidadeFormulario"
      );
  ...
  const btnFecharModalEditarFormulario = document.getElementById(
    "fecharModalEditarFormulario"
  );
  ...
  const btnFecharModalPrivacidadeFormulario = document.getElementById(
    "fecharModalPrivacidadeFormulario"
  );
  ...
  const btnSalvarPrivacidade = document.querySelector(
    "#modalPrivacidade button"
  );
  if (btnSalvarPrivacidade) {
    (btnSalvarPrivacidade as HTMLElement).onclick = async () => {
      ...
      if (radioSelecionado && arquivoPrivacidadeAtual) {
        const tipoPrivacidade =
          radioSelecionado.value === "publico" ? "público" : "privado";
        ...
        uiManager.mostrarPopup(
          `Anexo definido como ${tipoPrivacidade} com sucesso!`
        );
      } else {
        console.warn(
          "❌ Não conseguiu atualizar - radioSelecionado:",
          !!radioSelecionado,
          "arquivoPrivacidadeAtual:",
          !!arquivoPrivacidadeAtual
        );
      }
    };
  ...
  if (btnFecharPopup)
    btnFecharPopup.onclick = () => uiManager.togglePopupUpload(false);
  ...
  if (fileInput)
    fileInput.onchange = (e: any) =>
      iniciarUpload((e.target as HTMLInputElement).files);

  const btnSelecionarArquivo = document.getElementById(
  ...
  if (dropZone) {
    dropZone.ondragover = (e: any) => {
      e.preventDefault();
      dropZone.classList.add("drag-over");
    };
    dropZone.ondragleave = (e: any) => {
      e.preventDefault();
  ...
  const btnFecharEncaminhamento = document.getElementById(
    "fecharModalEncaminhamento"
  );
  if (btnFecharEncaminhamento)
    btnFecharEncaminhamento.onclick = () =>
      uiManager.toggleModalEncaminhamento(false);

  const btnFecharPrivacidade = document.getElementById(
    "fecharModalPrivacidade"
  );
  if (btnFecharPrivacidade)
    btnFecharPrivacidade.onclick = () =>
      uiManager.toggleModalPrivacidade(false);

  // Botão Salvar Alterações do Modal de Privacidade do Formulário
  const btnSalvarPrivacidadeForm = document.getElementById(
    "modal-privacidade-form"
  );
        ...
        padrao:
          (
            document.querySelector(
              'input[name="visibilidade-padrao"]:checked'
            ) as HTMLInputElement
          )?.id || "privado-padrao",
        assistida:
          (
            document.querySelector(
              'input[name="visibilidade-assistida"]:checked'
            ) as HTMLInputElement
          )?.id || "privado-assistida",
        caso:
          (
            document.querySelector(
              'input[name="visibilidade-caso"]:checked'
            ) as HTMLInputElement
          )?.id || "privado-caso",
        outras:
          (
            document.querySelector(
              'input[name="visibilidade-outras"]:checked'
            ) as HTMLInputElement
          )?.id || "privado-outras",
        profissional:
          (
            document.querySelector(
              'input[name="visibilidade-profissional"]:checked'
            ) as HTMLInputElement
          )?.id || "privado-profissional",
      };

      // Salva no localStorage
      localStorage.setItem(
        "configuracaoPrivacidadeFormulario",
  ...
  const atualizarDropdownModal = () => {
    ...
    uiManager.renderizarSelecaoModal(
      todosArquivos,
      estadoEncaminhamento.anexosSelecionadosIds,
      (idClicado: any) => {
        const index =
          estadoEncaminhamento.anexosSelecionadosIds.indexOf(idClicado);
        if (index === -1)
          estadoEncaminhamento.anexosSelecionadosIds.push(idClicado);
  ...
  document.addEventListener("click", (e: any) => {
    if (
      ...
      !menuAnexos.contains(e.target as Node) &&
      botaoDropdownAnexo &&
      !botaoDropdownAnexo.contains(e.target as Node)
    ) {
  ...
  if (btnEnviar) {
    btnEnviar.onclick = () => {
      const emailParaInput = document.getElementById(
        "email-para"
      ) as HTMLInputElement | null;
      const emailAssuntoInput = document.getElementById(
        "email-assunto"
      ) as HTMLInputElement | null;
      const emailCorpoInput = document.getElementById(
        "email-corpo"
      ) as HTMLInputElement | null;
      ...
      if (!emailCorpo || emailCorpo.trim().length < 5) {
        uiManager.mostrarPopup(
          "O campo de mensagem deve conter pelo menos 10 caracteres."
        );
        return;
      }
  ...
  if (redesContatadas && !redesContatadas.textContent.trim()) {
    redesContatadas.textContent =
      "Nenhuma rede de apoio foi contatada até o momento";
  }

  // Controle de Visibilidade Padrão e Etapas Específicas
  const radiosPadrao = document.querySelectorAll(
    'input[name="visibilidade-padrao"]'
  );
    ...
    radio.addEventListener("change", (e: any) => {
      ...
      radiosEtapas.forEach((etapa: any) => {
        if (isPublico && (etapa as HTMLInputElement).id.includes("publico")) {
          ...
          !isPublico &&
          (etapa as HTMLInputElement).id.includes("privado")
        ) {
          (etapa as HTMLInputElement).checked = true;
        }
      });
  ...
  if (btnAlterarPrivacidade) {
    btnAlterarPrivacidade.addEventListener("click", () => {
      ...
      const configSalva = localStorage.getItem(
        "configuracaoPrivacidadeFormulario"
      );
      if (configSalva) {
        try {
          ...
          if (config.assistida) {
            const radioAssistida = document.getElementById(
              config.assistida
            ) as HTMLInputElement;
            if (radioAssistida) radioAssistida.checked = true;
          }
          if (config.caso) {
            const radioCaso = document.getElementById(
              config.caso
            ) as HTMLInputElement;
            if (radioCaso) radioCaso.checked = true;
          }
          if (config.outras) {
            const radioOutras = document.getElementById(
              config.outras
            ) as HTMLInputElement;
            if (radioOutras) radioOutras.checked = true;
          }
          if (config.profissional) {
            const radioProfissional = document.getElementById(
              config.profissional
            ) as HTMLInputElement;
  ...
  const btnFecharInformacoes = document.getElementById(
    "fecharInformacoes"
  ) as HTMLButtonElement | null;
  if (btnFecharInformacoes) {
    btnFecharInformacoes.addEventListener("click", async () => {
      console.log(
        "[informaçõesCaso] Botão fechar clicado - voltando para casos da assistida ID:",
        idAssistida
      );

      // Guardar ID da assistida para retornar à lista de casos dela
      sessionStorage.setItem(
        "protocoloAssistidaSelecionada",
        String(idAssistida)
      );
      ...
      console.log(
        "[informaçõesCaso] Dados limpos do storage, voltando para assistida ID:",
        idAssistida
      );
