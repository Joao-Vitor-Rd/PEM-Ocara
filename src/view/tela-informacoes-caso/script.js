...
class UIManager {
  ...
  renderizarSelecaoModal(todosArquivos, selecionadosIds, onToggle) {
    ...
    const itens = [
      ...
      ...todosArquivos.filter((a) => a.status === "upado"),
    ];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (fecharMenuAnexos)
    fecharMenuAnexos.onclick = () => (menuAnexos.style.display = "none");
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      !menuAnexos.contains(e.target) &&
      !botaoDropdownAnexo.contains(e.target)
    ) {
    ...
    try {
      ...
      if (casoJSON) {
        ...
        uiManager.preencherDadosCaso({
          ...
          dataCadastro: dadosDoCaso.data
            ? new Date(dadosDoCaso.data).toLocaleDateString("pt-BR")
            : "",
  ...
  if (btnSalvarPrivacidadeForm) {
    btnSalvarPrivacidadeForm.addEventListener("click", () => {
      ...
      const configuracao = {
        padrao:
          document.querySelector('input[name="visibilidade-padrao"]:checked')
            ?.id || "privado-padrao",
        assistida:
          document.querySelector('input[name="visibilidade-assistida"]:checked')
            ?.id || "privado-assistida",
        caso:
          document.querySelector('input[name="visibilidade-caso"]:checked')
            ?.id || "privado-caso",
        outras:
          document.querySelector('input[name="visibilidade-outras"]:checked')
            ?.id || "privado-outras",
        profissional:
  ...
  if (btnAcessarForm) {
    btnAcessarForm.addEventListener("click", async () => {
      try {
        console.log("Abrindo Modo Visualização...");

        // --- PASSO 1: Preparar dados para a TELA 1 (Assistida) ---
        // Pegamos o ID da assistida que já está carregado na variável 'dadosDoCaso' da sua tela
        const idAssistida = dadosDoCaso.id_assistida || dadosDoCaso.assistidaId;

        if (idAssistida) {
          // Busca os dados completos da pessoa (endereço, idade, etc)
          ...
          if (resultado.success && resultado.assistida) {
            ...
            console.log("Dados da assistida salvos para visualização.");
          } else {
            throw new Error("Não foi possível buscar os dados da assistida.");
          }
        } else {
          throw new Error("ID da assistida não encontrado.");
        }
        ...
        if (dadosDoCaso && Object.keys(dadosDoCaso).length > 0) {
          ...
          sessionStorage.setItem("dadosCaso", JSON.stringify(dadosDoCaso));
          console.log("Dados do caso salvos para visualização (Telas 2 e 3).");
        } else {
          // Se por algum motivo a variável estiver vazia, tentamos buscar do banco novamente
          const idCaso =
            sessionStorage.getItem("idCasoAtual") || dadosDoCaso.id_caso;
          if (idCaso) {
            const infoCompleta = await window.api.getInformacoesGeraisDoCaso(
              Number(idCaso)
            );
            if (infoCompleta.success) {
              sessionStorage.setItem(
                "dadosCaso",
                JSON.stringify(infoCompleta.informacoes)
              );
            }
          } else {
            throw new Error("Dados do caso não estão carregados.");
          }
        }

        // --- PASSO 3: Ativar a Flag e Abrir ---
        sessionStorage.setItem("modoVisualizacao", "true");
        await window.api.openWindow("telaCadastroAssistida");
      } catch (erro) {
        console.error("Erro ao preparar visualização:", erro);
        if (typeof uiManager !== "undefined") {
          uiManager.mostrarPopup("Erro ao abrir visualização: " + erro.message);
        } else {
