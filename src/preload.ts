...
contextBridge.exposeInMainWorld("api", {
  ...
  buscarAssistidaPorId: (id: number) =>
    ipcRenderer.invoke("assistida:buscarPorId", id),

  atualizarAssistida: (data: any) =>
    ipcRenderer.invoke("assistida:atualizar", data),

  listarCasosPorAssistida: (idAssistida: number) =>
    ipcRenderer.invoke("caso:listarPorAssistida", idAssistida),

  criarCaso: (data: any) => ipcRenderer.invoke("caso:criar", data),
  ...
  registrarDelecaoAnexo: (
    ...
    ipcRenderer.invoke("historico:registrarDelecao", {
      idCaso,
      idAssistida,
  ...
  getTotalCasosNoAnoFiltrado: (
    regioes: string[],
    dataInicio?: string,
    dataFim?: string
  ) =>
    ipcRenderer.invoke("caso:getTotalCasosNoAnoFiltrado", {
      regioes,
      dataInicio,
      dataFim,
    }),
  ...
  getEnderecosAssistidasFiltrado: (dataInicio?: string, dataFim?: string) =>
    ipcRenderer.invoke("caso:getEnderecosAssistidasFiltrado", {
      dataInicio,
      dataFim,
    }),

  getTotalCasosFiltrado: (
    regioes: string[],
    dataInicio?: string,
    dataFim?: string
  ) =>
    ipcRenderer.invoke("caso:getTotalCasosFiltrado", {
      regioes,
      dataInicio,
      dataFim,
    }),
  ...
  listarOrgaosRedeApoio: () => ipcRenderer.invoke("orgao:listarTodos"),

  atualizarOrgaoRedeApoio: (id: number, nome?: string, email?: string) =>
    ipcRenderer.invoke("orgao:update", { id, nome, email }),

  deletarOrgaoRedeApoio: (id: number) => ipcRenderer.invoke("orgao:delete", id),
  ...
  excluirAnexo: (idAnexo: number, nomeArquivo?: string) =>
    ipcRenderer.invoke("caso:deletarAnexo", { idAnexo, nomeArquivo }),
  ...
  atualizarFuncionario: (
    email: string,
    dados: { nome?: string; cargo?: string; email?: string; senha?: string }
  ) => ipcRenderer.invoke("update-funcionario", { email, dados }),

  deletarFuncionario: (email: string) =>
    ipcRenderer.invoke("delete-funcionario", email),
